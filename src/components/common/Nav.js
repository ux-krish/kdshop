...
const Nav = ({onLogout, loggedIn}) => {
  ...
  return (
    ...
    <div className={`h-[60px] md:h-[72px] bg-slate-950/30 ${isOpen ? '' : 'backdrop-blur-xl'} fixed z-20 flex justify-between items-center inset-0 w-full`}>
      ...
      <div className={`mt-0 ml-4 fixed transform z-20 ease-in-out transition-all duration-300 ${
        ...
        <div
          className={`flex items-center justify-center w-12 h-10 cursor-pointer group rounded-full py-[2px] px-[0]  hover:bg-gradient-to-r from-pink-500 to-blue-500 hover:border-slate-950 ${isOpen ? 'md:bg-gradient-to-r from-pink-500 to-blue-500' : ''}`}
      ...
      <div className={`fixed left-[70px] md:inset-0 md:mx-auto h-[70px]  flex justify-center items-center ${isOpen ? '-z-50' : 'z-0'}`}>
        <Link to={'/'} className='font-black text-[24px] md:text-[40px] tracking-wide inline' onClick={() => setIsOpen(false)}>
